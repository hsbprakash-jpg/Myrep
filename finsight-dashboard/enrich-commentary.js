/* ================= Enrich commentary =====================================
   ...
   on. Accepted sentences join comInLoaded(), so the House commentary, the
   Financial Summary, All commentaries, the widgets and the exports weave
   them in exactly as they weave in the config's paragraphs — scope-
   following, digesting and the per-block cap included. The sources are
   named in a line under each commentary block; an inline marker on every
   clause is a checkbox away.
   Word files are unzipped natively (DecompressionStream); PDF is not read
   here — save it as Word or text.
   AI review (optional, off until switched on): after the rules have run, the
...
(function(){
  ...
  const EN={docs:[], prefer:false, mark:false,
    ai:{on:false, provider:'local', endpoint:'', model:'', key:'', threshold:0.7, rewrite:false}};
  ...
  const AI_DEFAULT={local:'http://localhost:11434/v1/chat/completions', lmstudio:'http://localhost:1234/v1/chat/completions',
                    anthropic:'https://api.anthropic.com/v1/messages'};
  function cfgAi(k,d){ try{ const v=cfgS('ai_'+k); return (v==null||String(v).trim()==='')? d : String(v).trim(); }catch(e){ return d; } }
  /* the page's own setting wins; the config pack's Settings sheet
     (ai_provider, ai_endpoint, ai_model, ai_key, ai_threshold, ai_rewrite)
     supplies the defaults so a team can be set up once */
  function aiCfg(){
    const a=EN.ai||{};
    const provider=a.provider||cfgAi('provider','local');
    const endpoint=a.endpoint||cfgAi('endpoint','')||(provider==='anthropic'? AI_DEFAULT.anthropic : AI_DEFAULT.local);
    const model=a.model||cfgAi('model','')||(provider==='anthropic'? 'claude-opus-5' : '');
    const key=a.key||cfgAi('key','');
    const th=parseFloat(a.threshold!=null? a.threshold : cfgAi('threshold','0.7'));
    const on=a.on!=null? !!a.on : /^(y|yes|on|true)$/i.test(cfgAi('provider','')? 'y':'');
    return {on, provider, endpoint, model, key, threshold:isFinite(th)? Math.min(1,Math.max(0,th)) : 0.7,
            rewrite:a.rewrite!=null? !!a.rewrite : /^y/i.test(cfgAi('rewrite','N'))};
  }
  ...
  async function aiTest(){
    ...
    if(!c.model){ out.textContent='Set a model name first.'; return; }
...
#enrichAi summary{cursor:pointer;font-size:11.5px;font-weight:600;color:var(--text-secondary)}
#enrichAi{border-top:1px solid var(--border);padding-top:6px;display:flex;flex-direction:column;gap:6px}
#enrichAi input[type=text],#enrichAi input[type=password],#enrichAi input[type=number],#enrichAi select{flex:1;min-width:0;border:1px solid var(--border);border-radius:6px;padding:4px 7px;font:inherit;font-size:12px;background:var(--surface-1);color:var(--text-primary)}
...
.en-nocue{font-size:10px;color:var(--text-muted);border:1px solid var(--border);border-radius:10px;padding:0 6px;margin-left:4px;white-space:nowrap}
    ...
    panel.innerHTML=`
      ...
      <details id="enrichAi"><summary>AI review <span class="muted" id="enrichAiSum"></span></summary>
        <label><input type="checkbox" id="aiOn"> let a model decide which sentences to use</label>
        <label>Runs on <select id="aiProvider">
          <option value="local">this desktop (Ollama / LM Studio)</option>
          <option value="openai">an approved gateway (OpenAI-style)</option>
          <option value="anthropic">Claude (Anthropic API)</option></select></label>
        <label>Endpoint <input type="text" id="aiEndpoint" placeholder="http://localhost:11434/v1/chat/completions"></label>
        <label>Model <input type="text" id="aiModel" placeholder="e.g. llama3.1:8b"></label>
        <label id="aiKeyRow">Key <input type="password" id="aiKey" placeholder="only for a gateway or Claude"></label>
        ...
        <div style="display:flex;gap:8px;align-items:center"><button class="btn-quiet" id="btnAiTest">Test</button><span class="note" id="enrichAiStatus"></span></div>
        <div class="note">Nothing leaves this machine when the model runs on the desktop. Ollama must be started with OLLAMA_ORIGINS=* so this page may call it; LM Studio needs CORS switched on.</div>
      </details>`;
    ...
    // and the commentary block has its own way in
    const hc=$id('houseCtl');
    if(hc&&!$id('btnHouseEnrich')){
      const b=document.createElement('button'); b.className='btn-quiet'; b.id='btnHouseEnrich';
      b.title='Add a write-up, notes or a transcript — its sentences enrich this commentary'; b.textContent='⤒ Enrich';
      hc.insertBefore(b, hc.querySelector('#btnHouseCopy')||null);
      b.addEventListener('click',e=>{ e.stopPropagation(); fi.click(); });
    }
    $id('enrichPrefer').addEventListener('change',e=>{ EN.prefer=e.target.checked; save(); applyChange(); });
    // AI settings: the page's own values, the config pack's as defaults
    const aiPaint=()=>{ const c=aiCfg();
      $id('aiOn').checked=c.on; $id('aiProvider').value=c.provider; $id('aiEndpoint').value=c.endpoint; $id('aiModel').value=c.model;
      $id('aiKey').value=c.key; $id('aiThreshold').value=String(c.threshold); $id('aiRewrite').checked=c.rewrite;
      $id('aiKeyRow').style.display=c.provider==='local'?'none':'';
      $id('enrichAiSum').textContent=c.on? `· on · ${c.model||'no model'} · ${c.provider==='local'?'desktop':c.provider}` : '· off';
    };
    const aiSet=(k,v)=>{ EN.ai=EN.ai||{}; EN.ai[k]=v; save(); aiPaint(); };
    $id('aiOn').addEventListener('change',e=>aiSet('on',e.target.checked));
    $id('aiProvider').addEventListener('change',e=>{ const p=e.target.value; EN.ai.provider=p;
      if(!EN.ai.endpoint||Object.values(AI_DEFAULT).includes(EN.ai.endpoint)) EN.ai.endpoint=(p==='anthropic'? AI_DEFAULT.anthropic : p==='local'? AI_DEFAULT.local : '');
      if(p==='anthropic'&&!EN.ai.model) EN.ai.model='claude-opus-5';
      save(); aiPaint(); });
    $id('aiEndpoint').addEventListener('change',e=>aiSet('endpoint',e.target.value.trim()));
    $id('aiModel').addEventListener('change',e=>aiSet('model',e.target.value.trim()));
    $id('aiKey').addEventListener('change',e=>aiSet('key',e.target.value.trim()));
    $id('aiThreshold').addEventListener('change',e=>aiSet('threshold',parseFloat(e.target.value)));
    $id('aiRewrite').addEventListener('change',e=>aiSet('rewrite',e.target.checked));
    $id('btnAiTest').addEventListener('click',aiTest);
    aiPaint();
