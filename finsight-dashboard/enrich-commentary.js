...
(function(){
  ...
  function openReview(docs){
    ...
    const body=docs.map((d,di)=>`
      <tr class="en-file"><td colspan="6">
        <b>${esc(d.name)}</b> <span class="muted">· ${d.kind==='transcript'?'transcript':'document'} · ${d.items.length} sentence${d.items.length===1?'':'s'}${d.items.some(i=>i.loose)? ', '+d.items.filter(i=>i.loose).length+' naming no line' : ''}</span>
  ...
  async function handleFiles(files){
    ...
    status((on? `${on} sentence${on===1?'':'s'} from ${who} ${on===1?'enriches':'enrich'} the commentary` : `Nothing in ${who} names a line of the loaded extract`)
      +(loose? ` · ${loose} naming no line kept unticked` : '')
