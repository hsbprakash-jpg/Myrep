...
.en-nocue{font-size:10px;color:var(--text-muted);border:1px solid var(--border);border-radius:10px;padding:0 6px;margin-left:4px;white-space:nowrap}
    ...
    // one line says what will decide; the fields under Advanced only override it
    const aiPaint=()=>{
      const c=aiCfg(), ln=$id('enrichAiLine'); ln.innerHTML='';
      const link=(t,f)=>{ const b=document.createElement('button'); b.className='linklike'; b.textContent=t; b.style.marginLeft='6px'; b.onclick=f; return b; };
      const txt=t=>ln.appendChild(document.createTextNode(t));
      if(c.source==='none'){
        txt(c.wanted? 'AI review: no model found on this desktop — the rules decide alone.' : 'AI review: off — the rules decide alone.');
        if(c.wanted){ ln.appendChild(link('Look again',async()=>{ txt(' …'); await aiProbe(); aiPaint(); })); ln.appendChild(link('Switch off',()=>{ EN.ai.on=false; save(); aiPaint(); })); }
        else ln.appendChild(link('Switch on',async()=>{ EN.ai.on=true; save(); await aiProbe(); aiPaint(); }));
      } else {
        const where=c.source==='desktop'? `on this desktop (${c.server})` : (c.provider==='anthropic'? 'via Claude' : 'via '+(c.endpoint.replace(/^https?:\/\//,'').split('/')[0]));
        txt(`AI review: ${c.on? '' : 'off · '}${c.model} ${where}${c.on? ' decides which sentences to use' : ''}.`);
        ln.appendChild(link(c.on? 'Switch off' : 'Switch on',()=>{ EN.ai.on=!c.on; save(); aiPaint(); }));
        if(c.source==='desktop') ln.appendChild(link('Look again',async()=>{ await aiProbe(); aiPaint(); }));
      }
      $id('aiEndpoint').value=EN.ai.endpoint||''; $id('aiModel').value=EN.ai.model||''; $id('aiKey').value=EN.ai.key||'';
      $id('aiThreshold').value=String(c.threshold); $id('aiRewrite').checked=c.rewrite;
      const s2=$id('enrichAdv').querySelector('summary'); s2.textContent='Advanced'+(EN.ai.endpoint? ' · custom endpoint' : '')+(c.rewrite? ' · reword' : '');
    };
    ...
    aiPaint();
    if(aiCfg().wanted!==false&&!EN.ai.endpoint) aiProbe().then(aiPaint);
